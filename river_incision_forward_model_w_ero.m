function [Z0,time_vec,Edt] = river_incision_forward_model_w_ero(S,S_DA,S_U,pSz,K,m,n,run_time,dt)
...
% (3) S_U - the uplift m/yr [Can be a scalar or vector]
% (4) pSz - pre-incision topography
...
% model run
% (2) time_vec - vector of model time in years
% (3) Edt - basin average erosion rate for each timestep
%
...
time_vec = (0:tsteps-1).*dt;

% allocate memory before running the model and calc. initial erosion rate
Z0 = S_Zi;
Edt = nan(tsteps,1);

% calculate dDA for basin aveerage erosion rate calculation
S_dDA = S_DA;
S_dDA(S.ixc) = S_dDA(S.ixc) - S_dDA(S.ix);
...
for t = 1:tsteps
    ...
    inci = (Z0-Z1)+(S_U).*dt; % local magnitude of incision over timestep
    Edt(t) = ((sum(inci(inci > 0).*S_dDA(inci > 0))))./sum(S_dDA)./dt;
